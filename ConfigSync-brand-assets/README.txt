...
- backdrop-ribbon.png is the source in use: a violet-blue ribbon curving out of the bottom-left.
  It is light painted on black, so backdrop.py *adds* it over a navy base rather than using it
  on its own — the ribbon keeps its curve and the navy shows through everywhere it is dark.
  Rebuild both derivatives with:  python3 ConfigSync-brand-assets/backdrop.py
