...
import { ConfigWall, DuotoneFilter, GameTexture, Grain } from "@/components/landing/backdrops";
...
/**
 * The landing page, "Two machines. One setup." `rich` adds the v3 background layers (config
 * wall, desk photo, game textures, grain) for the /v3 preview; / renders it without them.
 */
export function Landing({ rich = false }: { rich?: boolean }) {
  const cs2 = fileBacked("cs2");
  const rl = fileBacked("rocket-league");

  return (
    <div className={`${s.root} ${display.variable}`} data-rich={rich}>
      {rich ? (
        <>
          <DuotoneFilter />
          <Grain />
      ...
      <main>
        <HeroSync
          wall={rich ? <ConfigWall /> : undefined}
        ...
        <section id="games" className={s.section} data-scene="games">
          ...
          <div className={s.games}>
            ...
            >
              {rich ? <GameTexture game="cs2" /> : null}
            ...
            <Link href="/sign-up" className={`${s.gameTile} ${s.gameAny}`}>
              {rich ? <GameTexture game="any" /> : null}
